...
#+begin_src conf :tangle ./Dockerfile_rust_libtorch
  ...
  RUN \
      ...
      && cd libtorch \
      && cd lib \
      && ls | sed 's@^@(mv -vf -- "@g;s@$@" "/usr/lib/");@g' | sh \
      && cd .. \
      && rmdir lib \
      && cd include \
      && ls | sed 's@^@(mv -vf -- "@g;s@$@" "/usr/include/");@g' | sh \
      && cd .. \
      && rmdir include \
      && cd share/cmake \
      && ls | sed 's@^@(mv -vf -- "@g;s@$@" "/usr/share/cmake");@g' | sh \
      && cd ../.. \
      && rmdir -pv -- share/cmake bin \
      && rm 'build-hash' 'build-version' \
      && cd .. \
      && rmdir libtorch \
