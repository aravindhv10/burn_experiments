...
** Shell script to compile the model only when necessary
...
**** Implement methods for the functions
#+begin_src rust :mkdirp yes :tangle ./src/model.rs
  use std::ops::Index;
  impl arg_output {
      pub fn new() -> Self {
...
#+begin_src rust :mkdirp yes :tangle ./src/model.rs
  ...
  const CLASS_LABELS: [&str; SIZE_O] = ["empty", "occupied", "other"];
  ...
  impl prediction_probabilities_reply {
      ...
      pub fn from(input: arg_output) -> prediction_probabilities_reply {
          ...
          for i in 1..SIZE_O {
              ret.val[i] = input.val[i].to_string();
              if input.val[i] > input.val[max_index] {
                  max_index = i;
              }
          }
  ...
  pub struct InferRequest {
      img: image::RgbaImage,
      resp_tx: oneshot::Sender<Result<arg_output, String>>,
  }
  ...
  impl model_client {
      ...
      pub async fn do_infer_data(&self, data: Vec<u8>) -> Result<arg_input, String> {
