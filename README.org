...
*** OpenCV
- Decode binary image data to arg_input array.
- Memory management of binary data of image is on rust side.
- arg_input memory location is handeled on rust side.

*** Torch
- For now, arg_input memory management is done on rust side.
- Run inference on =device= using =libtorch=.
- Return arg_output const * which rust should neither own nor free.
...
#+begin_src emacs-lisp :results silent
  ...
  (async-shell-command "
      './.git.sh'
      git commit -m 'Trying more efficient use of vectors'
