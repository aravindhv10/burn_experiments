...
def compile_EP_to_tensorrt(path_file_input_EP_pt2, path_file_output_trt_pt2):
    ...
    # 1. Get the static dimensions from the exported program
    # Assuming input shape is (Batch, C, H, W)
    original_shape = list(ep.example_inputs[0][0].size())
    c, h, w = original_shape[1], original_shape[2], original_shape[3]

    # 2. Define the Dynamic Input
    # We set the batch dimension (index 0) to be dynamic
    input_specs = [
        torch_tensorrt.Input(
            min_shape=[1, c, h, w],         # Minimum batch size
            opt_shape=[8, c, h, w],         # Target/Most common batch size
            max_shape=[32, c, h, w],        # Maximum batch size
            dtype=torch.bfloat16,
            name="x",                       # Should match the input name in the graph
        )
    ]

    # 3. Update compile settings
    compile_settings = {
        "inputs": input_specs,              # Use the Input specs instead of a raw tensor
        "enabled_precision": {torch.bfloat16},
        "ir": "dynamo",
    }
    ...
    # For AOTI saving with dynamic shapes, we provide a sample input 
    # within the min/max range for the tracing logic
    example_input = torch.randn([8, c, h, w], dtype=torch.bfloat16, device="cuda")
...
#+begin_src emacs-lisp :results silent
  ...
  (async-shell-command "
      './.git.sh'
      git commit -m 'Added separate cmake lists file for trt'
